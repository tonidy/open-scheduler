...
func NewUpdateStatusService(grpcClient *sharedgrpc.GrpcClient, token string, nodeID string) (*UpdateStatusService, error) {
	if grpcClient == nil {
...
func (s *UpdateStatusService) Execute(ctx context.Context, nodeID string, token string) error {
	...
	if !resp.Acknowledged {
		return fmt.Errorf("update status rejected: %s", resp.ResponseMessage)
	}

	log.Printf("[UpdateStatusService] Status update successful: %s", resp.ResponseMessage)
	return nil
}
